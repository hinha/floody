...
func NewPlugin(opts ...Option) gorm.Plugin {
	...
	p.tracer = p.provider.Tracer("gorm")
...
func (p otelPlugin) Initialize(db *gorm.DB) (err error) {
	...
	hooks := []struct {
		...
		{cb.Create().Before("gorm:create"), p.before("INSERT"), "before:create"},
		{cb.Create().After("gorm:create"), p.after(), "after:create"},
		{cb.Create().After("gorm:commit_or_rollback_transaction"), p.commitOrRollback, "after:commit_or_rollback"},

		{cb.Query().Before("gorm:query"), p.before("SELECT"), "before:select"},
		...
		{cb.Delete().After("gorm:commit_or_rollback_transaction"), p.commitOrRollback, "after:commit_or_rollback"},

		{cb.Update().Before("gorm:update"), p.before("UPDATE"), "before:update"},
		{cb.Update().After("gorm:update"), p.after(), "after:update"},
		{cb.Update().After("gorm:commit_or_rollback_transaction"), p.commitOrRollback, "after:commit_or_rollback"},

		{cb.Row().Before("gorm:row"), p.before("QUERY"), "before:row"},
		{cb.Row().After("gorm:row"), p.after(), "after:row"},

		{cb.Raw().Before("gorm:raw"), p.before("QUERY"), "before:raw"},
...
func (p *otelPlugin) before(spanName string) gormHookFunc {
	return func(tx *gorm.DB) {
		...
		formatSpan := strings.ToUpper(tx.Dialector.Name())
		if tx.Statement.Table != "" {
			formatSpan += " " + tx.Statement.Table
		}
		formatSpan += " " + spanName
		ctx, _ = p.tracer.Start(ctx, formatSpan, trace.WithSpanKind(trace.SpanKindClient))
...
func (p *otelPlugin) commitOrRollback(tx *gorm.DB) {
	if tx.DryRun && !p.includeDryRunSpans {
		return
	}
